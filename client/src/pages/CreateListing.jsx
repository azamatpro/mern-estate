...
export default function CreateListing() {
  ...
  const handleImageSubmit = (e) => {
    if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
      ...
      setImageUploadError("You can upload 6 images per listings!");
      setUploading(false);
    }
  };
  ...
  const handleRemoveImage = (i) => {
    setFormData({ ...formData, imageUrls: formData.imageUrls.filter((_, index) => index !== i) });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (+formData.regularPrice < +formData.discountPrice)
        return setError("Discount price must lower than regular price!");
      setLoading(true);
      setError(false);
